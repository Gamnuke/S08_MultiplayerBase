...
/*
Called when join server button is pressed
*/
void UMainMenu::JoinServer() {
	if (MenuInterface == nullptr) { return; }

	// Check if tab is chosen
	if (SelectedIndex.IsSet() && SessionTabs.IsValidIndex(SelectedIndex.GetValue())) {
		...
		for (FOnlineSessionSearchResult Element : SearchResult->SearchResults) {
			...
			if (Element.Session.SessionSettings.Get(TEXT("ServerLocked"), ServerLocked)) {
				...
				if (Tab->LockedImage != nullptr) {
					Tab->LockedImage->SetVisibility(ServerLocked ? ESlateVisibility::Visible : ESlateVisibility::Hidden);
				}
			}
...
void UMainMenu::SetIndex(USessionTab *OwningTab, int Index) {
	SelectedIndex = Index;
